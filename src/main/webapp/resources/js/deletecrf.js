var app = angular.module('crfApp', [ 'ui.bootstrap' ])
var modalTemplate = 
	...
	+ "</div>"
+"<div class='modal-body'><span><i style= color:#CB2020;' class='glyphicon glyphicon-remove w3-spin w3-jumbo'></i> Are you sure ! You want to Delete this CRF <span style= color:blue;'><b> ({{crf_Number}})</b></span> associated to Project <span style= color:blue;'><b>({{project_Number}})</b></span> ?</span></div>"
...
app.controller('crfCtrl', function($scope, $http, $uibModal) {
	...
	$scope.openDeleteModal = function(id,crf_Number,project_Number) {
...
app.controller('deleteModalController', function($scope, $http, $uibModalInstance, id,project_Number,crf_Number) {
	console.log('delete', id);


	$scope.id = id;
	$scope.project_Number=project_Number;
	$scope.crf_Number=crf_Number;
	$scope.delete = function() {
		...
		$http.post('/AizantCaseReportApplication/deletecrf?crfId=' + $scope.id).then(function(response) {
